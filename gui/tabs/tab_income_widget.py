...
from models import session, func, distinct, Income, sum_income
...
class TabIncome(QWidget):
    def __init__(self):
        ...
        n_row = session.query(func.count(distinct(Income.id))).scalar() // ROW_SET + 1
        ...
        self.init_table(sum_income())
        self.display(session.query(Income).all())
